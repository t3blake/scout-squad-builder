...
function buildFiles(values) {
  ...
  const manifest = {
    ...
    description: `${values.ownerName}'s Scout squad for ${values.focus}`,
    timeoutSeconds: 300,
...
You are Squad Lead for ${values.ownerName} (${values.ownerRole}).
...
form.addEventListener("submit", async (event) => {
  ...
  const values = collectValues();
  if (!values.squadName || !values.ownerName || !values.ownerRole || !values.focus) {
    alert("Please fill in squad name, owner, role, and focus.");
    return;
  }
